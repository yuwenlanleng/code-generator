...
import com.gome.generator.utils.MyExcelHelper;
...
public class MainWindow extends java.awt.Frame {
	...
	private void btnBuildEntityActionPerformed(java.awt.event.ActionEvent evt) {
		...
		for (int i = 0; i < tabTables.getRowCount(); i++) {
			...
			if (isSelect) {
				...
				CodeAutoGenerator.generator(excelTemps, true, "", "v4");
				print("生成[OK]\r\n");
			} else {
				print("未选择需要生成的表\r\n");
			}
	...
	private void initTabTable(String str) {
		...
		for (Map<String, Object> key : excelTables) {
			...
			// }
			// }
			GeneratorInf af = (GeneratorInf) key.get(GeneratorInf.AUTOGENINF);
			TableInf tbinf = (TableInf) key
					.get(TableInf.TABLEINFENTITY);
			arrTemp.add(new Object[] { true, tbinf.getTableName(),
					tbinf.getDescription(), af.getPackageName(), key });
		}
	...
	private void initComponents() {
		...
		tabTables.setModel(new DefaultTableModel(
